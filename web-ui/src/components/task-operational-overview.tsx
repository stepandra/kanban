...
export function TaskOperationalOverview({
	...
	const agentLabel =
		getRuntimeAgentCatalogEntry(selection.card.agentId ?? "codex")?.label ?? selection.card.agentId ?? "Default worker";
	const status = isStaleAttempt ? "stale" : execution?.status ?? (selection.card.execution ? "checking" : "not queued");

	return (
		<section className="shrink-0 border-b border-border bg-surface-1 px-4 py-3" aria-label="Task operational overview">
