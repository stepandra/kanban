...
- Amp thread IDs and immutable task origin are provenance, not acceptance authority. The current Amp tool context has no opaque actor capability that Kanban can verify, so per-task read-only acceptance must remain fail-closed. Do not reintroduce it through CLI or browser fields, environment/internal bearer markers, hidden commands, or prompt policy; preserve Review until a genuine Architect-owned channel exists.
