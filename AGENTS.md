...
- The accepted worker target is harness-centric, not model-centric: Grok Build is the primary implementation harness and owns LLM Gateway routing, model roles, subagents, and Rhai workflows. Amp owns Architecture/Product/UI-UX planning and one `a1.xxlarge` integration/QA/production Orb per frozen campaign; `agentId=amp`, per-task Amp Orbs, per-task Fixers, and per-task acceptance are legacy paths to remove. Remote Grok execution uses ACP sessions; SSH/Zellij/terminal/workflow state is telemetry, never lifecycle truth. See `docs/decisions/2026-08-11-grok-build-workers-and-qa-campaigns.md`.
- Workspaces are becoming durable, host-qualified operational objects. The target UI inventories every non-deleted task or campaign workspace with host, path, associated task/campaign IDs, revision, lifecycle, and observed presence. Do not infer workspace history from a task ID or treat a host report of `missing` as authoritative deletion.
- The target browser starts from global Tracks, which rolls up delivery scope, remote executions, candidates, campaigns, and blockers across projects. Board remains a tactical project task editor; Workspaces remains the physical inventory. A QA campaign is an explicit frozen candidate set inside one project/repository and may span Tracks/milestones there. The supported Zellij Focus Cockpit always keeps Amp Architect on the left and an identity-driven stack of selected In Progress task executions and QA campaigns on the right; never add fixed per-harness slots, newest-zmx discovery, or pane-driven lifecycle.
...
- For exact jj working-copy inspection in a colocated repository, run `jj --no-pager` from the target workspace directory. Do not use `jj -R <path>` for these reads: repository selection can resolve a different workspace backed by the same store.
- Local single-user task acceptance trusts `kanban task accept` invoked by the local OS user. The installed `kanban_tasks` Amp plugin is the normal caller and must pass the exact current `PluginToolContext` thread ID; Kanban matches it to immutable task origin while re-verifying task fences and local workspace or report receipts. There is deliberately no second caller-authentication mechanism, so direct local CLI use has the same authority. Browser snapshot saves, environment markers, bearer tokens, hidden commands, and prompt policy are not acceptance authority.
