...
license: Apache-2.0
compatibility: Requires the local stepandra/kanban CLI and its Amp plugin; durable interactive agents additionally require zmx.
metadata:
...
- Kanban's terminal runtime owns interactive agent processes through durable zmx sessions.
- Zellij is an optional, replaceable view of those sessions. Terminal output,
  process exit, zmx state, and cockpit indicators never complete or accept a task.
